...
const Navbar = () => {
  ...
  //   return (
  // <div className="flex justify-between items-center w-full h-20 text-white fixed bg-gradient-to-r from-cyan-500 to-blue-500 px-4">
  //   <div>
  //     <h1 className="text-5xl font-signature ml-2">Rahat</h1>
  //   </div>
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500  fixed">
      ...
      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-indigo-500 via-purple-500 to-pink-500 text-white">
          {links.map(({ id, link }) => (
            <li
              key={id}
